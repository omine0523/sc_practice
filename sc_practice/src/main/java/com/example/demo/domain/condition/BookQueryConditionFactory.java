...
/**
 * 画面で入力された検索条件を業務ロジック用・テーブル検索等で使用できるようformクラスから切り離して整理・組み立てを行う Factory クラス
 */
@Component
public class BookQueryConditionFactory {

	/**
	 * 各条件で入力値・選択がある場合にのみ入力値を変数に入れ替えてフォームと切り離してオブジェクトの組み立てを行う。
	 * 
	 * @param form 書籍検索画面で指定した検索条件を受け取る
	 * @return ドメイン用の検索条件オブジェクト
	 */
	public BookQueryCondition createCondition(BookQueryForm form) {
		...
		// フォームで受け取ったジャンルIDをそのままドメイン用に使用する。
		Integer genreId = form.getGenreId();
		
		// フォームで受け取った置き場所IDをそのままドメイン用に使用する。
		Integer storageLocationId = form.getStorageLocationId();

		// 画面で入力された検索条件をもとに、
		// 業務ロジック・DB検索で使用できる検索条件オブジェクトを Builder で生成する。
		// ・入力がある項目のみを Condition に設定する。
		// ・すべて未指定の場合は build() 内で例外が発生する。
		return BookQueryCondition.builder()
				.bookId(bookId) // 正規化・数値変換済みの書籍ID（未入力なら null）
				.bookName(bookName) // 先頭・末尾の空白を削除した後の書籍名（未入力なら null）
				.fkGenreId(genreId) // ジャンルID（未選択なら null）
				.fkStorageLocationId(storageLocationId) // 置き場所ID（未選択なら null）
				.build(); // 検索条件オブジェクトを生成する。
	}
}